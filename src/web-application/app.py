...
def downloads_pk():
	...
	if(os.path.isfile("./media/database/database_1.pickle")):
		...
		pickleObj.close()
	if len(username) == 0:
		return render_template('public-key-list.html',msg='Aww snap! No public key found in the database. Seems like nothing is stored in thr directory yet.')
	else:
...
def download_f():
	for root,dirs,files in os.walk(UPLOAD_FOLDER):
		if(len(files) == 0):
			return render_template('file-list.html',msg='Aww snap! No file found in directory. SEems like no file is stored in the directory.')
		else:
